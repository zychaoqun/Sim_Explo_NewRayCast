close all;
clear all;
TrainFolderDungeon = 'TrainImg_Dungeon_11224_360heading';
TestFolderDungeon = 'TestImg_Dungeon_11224_360heading';
...
if(trainORtest)
    ...
    if ( (row_lb + row_ub >= 2 * SensorRange ) &&  (col_lb + col_ub >= 2 * SensorRange ))
        ...
        imwrite(local_img, strcat([TrainImgPath img_name '_'  num2str(Step_Counter) '|' num2str((np_idx-1)*10) ]) , 'jpg');
        fprintf(trainFileID,'%s_%d|%d %d\n', img_name, Step_Counter, (np_idx-1)*10, (np_idx-1)*10);
    else
    ...
    if ( (row_lb + row_ub >= 2 * SensorRange ) &&  (col_lb + col_ub >= 2 * SensorRange ))
        local_img = OP_MAP(im_row-SensorRange+1:im_row+SensorRange,im_col-SensorRange+1:im_col+SensorRange);
        imwrite(local_img, strcat([TestImgPath img_name '_'  num2str(Step_Counter) '|' num2str((np_idx-1)*10) ]) , 'jpg');
